...
# ─── LEMON SQUEEZY: RESUME PDF PAYMENT GATE ───
LEMONSQUEEZY_WEBHOOK_SECRET = os.environ.get('LEMONSQUEEZY_WEBHOOK_SECRET', '').strip()

# Simple in-memory store: session_id -> timestamp when marked paid.
# Good enough at current traffic. If Render restarts mid-checkout, that one
# session would need a retry — acceptable tradeoff for now, revisit with a
# real DB once volume justifies it.
paid_sessions = {}

def verify_lemonsqueezy_signature(raw_body, signature_header):
...
@app.route('/webhook/lemonsqueezy', methods=['POST', 'OPTIONS'])
def lemonsqueezy_webhook():
    ...
    if event_name == 'order_created':
        ...
        if session_id:
            paid_sessions[session_id] = time.time()
            print(f"Payment confirmed for session: {session_id}")

    return jsonify({"status": "ok"}), 200


@app.route('/check-payment/<session_id>', methods=['GET', 'OPTIONS'])
